...
def gpt_summary(query,model,language):
    if language == "zh":
        messages = [
            ...
            {"role": "assistant", "content": f"请用中文总结这篇文章，先提取出最多{keyword_length}个关键词，在同一行内输出，然后换行，用中文在{summary_length}字内写一个包含所有要点的总结，按顺序分要点输出，并按照以下格式输出'<br><br>总结:'，<br>是HTML的换行符，输出时必须保留2个，并且必须在'总结:'二字之前"}
        ]
    else:
        messages = [
            {"role": "user", "content": query},
...
#                entry.published = parse(entry.published).strftime('%a, %d %b %Y %H:%M:%S %z')
            ...
            if cnt > max_items:
                ...
                try:
                    ...
                    try:
                        entry.summary = gpt_summary(cleaned_article,model="gpt-4-0125-preview", language=language)
                        with open(log_file, 'a') as f:
                            f.write(f"Token length: {token_length}\n")
                            f.write(f"Summarized using GPT-4-0125-preview\n")
